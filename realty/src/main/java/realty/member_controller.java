...
public class member_controller {
	...
	public String reservation_delete(HttpSession se, @RequestParam int ridx, HttpServletResponse res)
			throws IOException {
	...
	public String md_board_view(@ModelAttribute(name = "dto") md_choice_dto dto, Model m, HttpServletRequest req,
	...
	public void search_mypassok(HttpSession se, HttpServletResponse res, @RequestParam("mpass") String mpass)
			throws Exception {
	...
	public String search_pass(@ModelAttribute(name = "dto") member_dto dto, HttpServletResponse res, HttpSession pw)
			throws Exception {
	...
	public String search_email(@ModelAttribute(name = "dto") member_dto dto, HttpServletResponse res, HttpSession se)
			throws Exception {
	...
	public String loginok(@RequestParam("memail") String memail,
			...
			@RequestParam(name = "login_channel", required = false) String login_channel, HttpSession se, Model m,
			member_dto dto) {
		...
		try {
			...
			if (login_channel.equals("1")) { // 웹페이지 회원
				...
				if (loginfo != null) { // 회원일 시 session 등록
					...
					msg = "alert('카카오 사용자로 로그인 시 간편회원가입이 필요합니다.');" + "sessionStorage.setItem('mid','" + kakao_id
